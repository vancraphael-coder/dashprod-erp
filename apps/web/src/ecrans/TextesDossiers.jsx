...
export default function TextesDossiers({ retour }) {
  const [stockes, setStockes] = useState(null);
  ...
  if (groupe) {
    return <SousPage groupe={groupe} stockes={stockes} org={org}
    ...
    <div style={S.page}>
      <div style={S.entete}>
        ...
        <div style={{ fontSize: 12, color: C.muet, marginTop: 2 }}>
          Tous les textes qu'un client peut lire. Un champ vide garde le texte par défaut.
        </div>
      </div>
          ...
          return (
            <button key={g.cle} onClick={() => setOuvert(g.cle)} style={carteBouton}>
              ...
              <span style={{ flex: 1, minWidth: 0 }}>
                <span style={{ display: "block", fontSize: 14, fontWeight: 700, color: C.encre }}>{g.titre}</span>
                <span style={{ display: "block", fontSize: 11.5, color: C.muet, marginTop: 2, lineHeight: 1.4 }}>{g.resume}</span>
                {!g.fichier && (
                  <span style={{ display: "inline-block", marginTop: 6, fontSize: 10.5,
                    ...
                    {perso ? `${perso} texte${perso > 1 ? "s" : ""} personnalisé${perso > 1 ? "s" : ""}` : "textes par défaut"}
                  </span>
                )}
              </span>
      ...
      {erreur && <div style={{ margin: "0 16px 24px", fontSize: 12.5, color: C.rouge }}>{erreur}</div>}
    </div>
  );
}
...
function SousPage({ groupe, stockes, org, onStockes, retour }) {
  ...
  const [valeurs, setValeurs] = useState(() => lireGroupe(stockes, groupe));
  ...
  async function enregistrer() {
    setErreur(null);
    try {
      const complet = ecrireGroupe(stockes, groupe, valeurs);
  ...
  async function deposerCbd(fichier) {
    ...
    try {
      ...
      setErreur(e.message || "Dépôt impossible");
    }
    setEnvoiCbd(false);
  }
    ...
    <div style={S.page}>
        ...
        <div style={S.carte}>
          {groupe.champs.map((ch) => (
            <div key={ch.cle} style={{ marginBottom: 10 }}>
              <label style={S.label}>
                {ch.label}
                {ch.aide && <span style={{ fontWeight: 500, color: C.fantome, marginLeft: 6 }}>{ch.aide}</span>}
              </label>
              {ch.long ? (
                <textarea style={{ ...S.input, minHeight: 46 }} value={valeurs[ch.cle] ?? ""}
                          placeholder={String(defauts[ch.cle] ?? "")}
                          onChange={(e) => maj(ch.cle, e.target.value)} />
              ) : (
                <input style={S.input} type={ch.nombre ? "number" : "text"}
                       value={valeurs[ch.cle] ?? ""}
                       placeholder={String(defauts[ch.cle] ?? "")}
                       onChange={(e) => maj(ch.cle, ch.nombre ? (e.target.value === "" ? "" : Number(e.target.value)) : e.target.value)} />
              )}
            </div>
          ))}
        </div>
      ...
      {apercu && (
        <div style={S.carte}>
          ...
          <pre style={{
            ...
            color: C.encre, whiteSpace: "pre-wrap", fontFamily: "ui-monospace, monospace",
          }}>{apercu.corps}</pre>
        </div>
      )}
      ...
      {groupe.fichier && (
        <div style={S.carte}>
          ...
          {cbd === undefined ? null : cbd ? (
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 10, background: "#ECFDF5", border: "1px solid #A7F3D0" }}>
              <span style={{ fontSize: 12.5, fontWeight: 700, color: "#065F46" }}>✓ Conditions déposées</span>
              <a href={cbd} target="_blank" rel="noreferrer" style={{ fontSize: 12.5, fontWeight: 700, color: C.bleu }}>Ouvrir</a>
            </div>
          ) : (
            <div style={{ padding: "10px 12px", borderRadius: 10, background: "#FFFBEB", border: "1px solid #FDE68A", fontSize: 12.5, color: "#92400E" }}>
              Aucun document déposé — les offres partiront sans les conditions.
            </div>
          )}
          {modeDonnees() === "reel" && (
            <label style={{ ...S.boutonLien, display: "block", marginTop: 10, cursor: "pointer", fontWeight: 700 }}>
              {envoiCbd ? "Dépôt en cours…" : cbd ? "Remplacer le PDF" : "Déposer le PDF"}
              <input type="file" accept="application/pdf" style={{ display: "none" }} onChange={(e) => deposerCbd(e.target.files?.[0])} />
            </label>
          )}
        </div>
      )}
      ...
      {groupe.champs.length > 0 && (
        <div style={{ margin: "0 16px 24px" }}>
          <button style={S.boutonPlein} onClick={enregistrer}>{sauve ? "✓ Enregistré" : "Enregistrer"}</button>
          {Object.keys(valeurs).length > 0 && (
            <button onClick={reinitialiser} style={{ ...S.boutonLien, display: "block", width: "100%", marginTop: 10, fontWeight: 600 }}>
...
const carteBouton = {
  display: "flex", alignItems: "flex-start", gap: 12, width: "100%", marginTop: 10,
  padding: 14, border: `1px solid ${C.bord}`, borderRadius: 14, background: C.blanc,
