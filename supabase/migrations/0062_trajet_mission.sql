-- =============================================================================
-- 0062_trajet_mission.sql
--
-- ⚠ RESTAURÉE LE 2026-09-19 depuis le registre de migrations de la base de
--   production (`supabase_migrations.schema_migrations`). Ce fichier avait été
--   appliqué en production le 2026-07-29 mais n'avait jamais été commité : le
--   dépôt ne pouvait donc pas reconstruire la base. Contenu repris tel quel.
--
...
--
-- `trajet_source` dit d'où vient le chiffre, et l'écran le répète à
-- l'utilisateur :
--   'mesure'  → renvoyé par un service de routage
--   'estime'  → déduit d'une distance et d'une vitesse moyenne
--   'manuel'  → saisi par le bureau
-- Sans valeur, on n'affiche aucune heure de départ : mieux vaut ne rien dire
-- que conseiller un horaire inventé.
-- =============================================================================
...
begin
  ...
  if p_minutes is not null and (p_minutes < 0 or p_minutes > 24 * 60) then
    return jsonb_build_object('ok', false,
      'message', 'Durée de trajet invalide.');
  end if;
