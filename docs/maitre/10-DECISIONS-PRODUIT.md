...
## Landing — structure visuelle des offres sectorielles (01/09/2026)

- **Domaine** (commercial/plans.js) : OFFRES_SECTEURS + statut à 3 valeurs
  (disponible / bientot / etude). `offreSouscriptible` traduit en CODE la règle
  « rien ne se vend avant d'exister » ; `offresVitrine` cache l'étude et trie du
  moins cher au plus cher. Éprouvé par sabotage (2).
- **Vitrine** : nouveau CarteOffreSecteur.jsx + section « écosystème » dans la
  Landing, distincte des paliers déménageur (ce sont des MÉTIERS, pas des
  tailles : ni hiérarchie, ni « gain sur le précédent »). Badge « Bientôt »
  visible, bouton « Être prévenu au lancement » — JAMAIS « souscrire » sur un
  parcours inexistant. Entrée de nav « Réseau ».
- Chaque carte vend ses RÉCURRENTS (« Ne sera plus jamais contesté »), pas un
